...
def main(sensor_id, amp, nsteps, nwalkers):
    ...
    overscan_fitting = OverscanFitting(params0, constraints, JointSimulatedModel, start=1, stop=4)
    ...
    outfile = '{0}_Amp{1}_lowtrap_mcmc.fits'.format(sensor_id, amp)
    save_mcmc_results(sensor_id, amp, sampler.chain, outfile, LinearTrap)

    samples = sampler.chain.reshape((-1, 4))
    fig = corner.corner(samples)
    fig.savefig('{0}_Amp{1}_lowtrap_triangle.png'.format(sensor_id, amp))
